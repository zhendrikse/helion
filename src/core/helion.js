...
class SimulationClock {
    constructor({ realTimeStep = 0.01, simulationTimeStep = realTimeStep, maxAccumulatedTime = 0.25 } = {}) {
...
export class Simulation {
    ...
    static with({
        ...
        camera = { position: new Vec3(3, 3, 3), target: new Vec3(0, 0, 0), fieldOfView: 50, controls: true, autoRotate: false, orthographic: false },
        scene = { background: ThreeJsScene.Background.TRANSPARENT, backgroundColor: 0x0088ff, scale: 1 },
        ...
        const renderer = new ThreeJsRenderer({ camera, viewport: viewPort, lighting, scene });
    ...
    constructor(viewport, renderer, headUpDisplay, infoPanel) {
        ...
        if (headUpDisplay) this._initHud()
        if (infoPanel.text) this._viewport.infoPanelText = infoPanel.text;
    ...
    addObject3D(object3D) { this._renderer.add(object3D); return this; }
    removeAxes() { this._renderer.removeAxes(); return this; }
    setLatexTitle(latex) { renderMath(this._viewport.titleDiv, latex); return this; }
    setTextTitle(text) { this._viewport.titleDiv.textContent = text; return this; }
    clearTitle() { this._viewport.titleDiv.replaceChildren(); return this; }
    runsEvery(dt) { this._clock.realTimeStep = dt; return this; }
    advancesBy(dt) { this._clock.simulationTimeStep = dt; return this; }
    bind(binding) {
        const existingIndex = this._bindings.findIndex(b => b.view === binding.view);
        if (existingIndex >= 0) {
    ...
    frameSceneOn(anObject, { padding = 1.2, translationY = 0, minDistance = 2, viewDirection = new Vec3(1, 1, 1) } = {}) {
        if (!anObject.boundingBox) throw new Error('Trying to provide axes for an object that does not have a boundingBox property');
        this._renderer.frameSceneOn(anObject, { padding, translationY, minDistance, viewDirection });
        return this;
    }
    provideAxesAround(anObject, { layoutType = Axes.Type.MATLAB, divisions = 10, frame = true, annotations = true, tickLabels = true, xyPlane = true, xzPlane = true, yzPlane = true, axisLabels = ["X", "Y", "Z"], positiveXZ = false, bottomAlign = true } = {}) {
        if (!anObject.boundingBox) throw new Error('Trying to provide axes for an object that does not have a boundingBox property');
        const axes = this._renderer.provideAxesAround(anObject, { layoutType, divisions, frame, annotations, tickLabels, xyPlane, xzPlane, yzPlane, axisLabels, positiveXZ, bottomAlign });
        if (this._axesUI) this._axesUI.axes = axes;
        else { this._axesUI = new AxesUI(axes); this.append(this._axesUI.ui()) }
        return this;
    }
    ...
    _tuneIterationsPerFrame(timeStamp) {
        if (this._framesPerSecond < this._minimumFrameRate) this._iterationsPerFrame--; else this._iterationsPerFrame++;
        this._framesPerSecond = 0;
    ...
    animate = (timeStamp) => {
        if (this._status === Simulation.Status.RUNNING) {
            if (this._maxPerformanceFunction) {
                if (timeStamp - this._lastTime > 1000) this._tuneIterationsPerFrame(timeStamp);
                let iterations = 0;
                while (iterations < this._iterationsPerFrame) { this._maxPerformanceFunction(this._clock); iterations++; }
                this._framesPerSecond++;
            }
            if (this._stepFunction) { this._clock.updateWith(timeStamp, this._timeScale); this._updatePhysics(); }
        }
        this._onFrame(timeStamp);
        for (const binding of this._bindings) binding.synchronize();
    ...
    defaultMouseClickCallback(event) {
        if (this._status === Simulation.Status.STOPPED) { this._hud?.show("Running", 1000); this._status = Simulation.Status.RUNNING; }
        else if (this._status === Simulation.Status.RUNNING) { this._hud?.show("Click to reset the simulation"); this._status = Simulation.Status.PAUSED; }
        else if (this._status === Simulation.Status.PAUSED) { this.reset(); this._hud?.show("Click to restart the simulation"); this._status = Simulation.Status.STOPPED; }
    }
    ...
    appendStartStopResetUI() {
        ...
        runButton.addEventListener("click", () => { if (this._status === Simulation.Status.RUNNING) { this._hud?.show("Paused"); runButton.withText("▶︎ Run"); this.stop(); } else { this._hud?.show("Running", 1000); runButton.withText("❚❚ Pause"); this.start(); } })
            .togetherWith(new Button().addEventListener("click", () => { this._hud?.show("Reset", 1000); this.reset(); }).withText("⟳ Reset"));
    ...
    setupGraphWith({ dataDefinition, width = this._viewport.width, height = this._viewport.height, title = "", xLabel = "", yLabel = "", maxPoints = 500, labelColor = "green" } = {}) {
        const plotParentDiv = this._viewport.addOnsDiv;
        this._plot = new UPlotGraph({ plotParentDiv, dataDefinition, width, height, title, xLabel, yLabel, maxPoints, labelColor });
        return this;
    }
    plot(variables) {
        for (let i = 0; i < variables.length; ++i) this._plot.graphData[i].push(variables[i]);
        this._plot.update();
    }
    setPlotData(data) {
